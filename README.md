...
### API: What It Is and How It Works Here
#### What Is an API?
An API (Application Programming Interface) enables different software applications to communicate. Here, Quotable API acts as an endpoint for fetching quote data.

#### How APIs Work
In simple terms, the API waits for a GET request (a type of HTTP request method) from the application, which requests data. Once received, it sends back a JSON response containing the quote data.

#### Quotable API in This Project
The Quotable API, created by Luke Peavey, is an open-source, free-to-use RESTful API. Each request to the API returns data on random quotes, including the quote’s text and author’s name.
...
#### Requests
- Created by: Kenneth Reitz, 2011
- Purpose: Requests is used to make HTTP requests (like API calls) simpler by handling complexities such as headers and status codes.
- How It Works in This Project: The library’s requests.get() function is used to fetch quotes, sending data back in JSON format.
...
The command `pip install requests` installs the **requests** package, while `pip install pyttsx3` installs the **pyttsx3** library. Each `pip install` command goes to Python’s official package repository, **PyPI (Python Package Index)**, and installs the latest version of the specified library.
```
...
- requests: Handles API calls to get quotes.
